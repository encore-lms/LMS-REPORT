...
sections/NN-제목.html 조각을 파일명 순서대로 sections/_shell.html 의
...
def main():
    shell = (SECTIONS / '_shell.html').read_text(encoding='utf-8')
    assert MARKER in shell, f'_shell.html 에 {MARKER} 가 없다'

    files = sorted(p for p in SECTIONS.glob('[0-9]*.html'))
